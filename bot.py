...
HELSINKI_UTC_OFFSET = 3

# Daily prize pool
DAILY_PRIZES = {1: 4.0, 2: 2.0, 3: 1.0}
DAILY_PRIZE_DEFAULT = 0.5
...
# FIX 1: নতুন button order
# বাম দিক:  Balance, Withdraw, Currency, Referrals, Support
# ডান দিক:  Tasks → Language → Top → Submissions → Profile
def main_menu_keyboard(chat_id):
    ...
    if is_bn(chat_id):
        ...
        kb.row("📤 উত্তোলন", "🌐 ভাষা")
        kb.row("💵 কারেন্সি", "🏆 টপ")
        kb.row("👥 রেফারেল", "📁 সাবমিশন")
        kb.row("📞 সাপোর্ট", "👤 প্রোফাইল")
    else:
        kb.row("💰 Balance", "📋 Tasks")
        kb.row("📤 Withdraw", "🌐 Language")
        kb.row("💵 Currency", "🏆 Top")
        kb.row("👥 Referrals", "📁 Submissions")
        kb.row("📞 Support", "👤 Profile")
    return kb
...
def instagram_type_inline(currency="BDT", chat_id=None):
    ...
    kb.add(
        types.InlineKeyboardButton("⚙️ Set Admin Rate", callback_data="adminrate_instagram"),
...
def facebook_type_inline(currency="BDT", chat_id=None):
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(
        types.InlineKeyboardButton("⚙️ Set Admin Rate", callback_data="adminrate_facebook"),
        types.InlineKeyboardButton(
...
def gmail_type_inline(currency="BDT", chat_id=None):
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(
        types.InlineKeyboardButton("⚙️ Set Admin Rate", callback_data="adminrate_gmail"),
        types.InlineKeyboardButton(
...
# FIX 2: Withdrawal method inline — শুধু একটা method select হবে, double message হবে না
def withdraw_method_inline(currency="BDT"):
    kb = types.InlineKeyboardMarkup(row_width=1)
    if currency == "BDT":
        kb.add(
            types.InlineKeyboardButton("bKash", callback_data="wmethod_bkash"),
            types.InlineKeyboardButton("Nagad", callback_data="wmethod_nagad"),
        )
    else:
        kb.add(
            types.InlineKeyboardButton("USDT (BEP-20)", callback_data="wmethod_usdt_bep"),
        )
    return kb
...
# FIX 3: referral_menu_inline — এই function টা আগে missing ছিল
def referral_menu_inline(chat_id):
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        types.InlineKeyboardButton(
            "📋 নিয়মাবলী" if is_bn(chat_id) else "📋 Rules",
            callback_data="ref_rules"
        ),
        types.InlineKeyboardButton(
            "🏆 টিম লিডারবোর্ড" if is_bn(chat_id) else "🏆 Team Leaderboard",
...
def handle_approve(message):
    ...
    try:
        ...
        payout_str    = f"+৳{total_payout_bdt:.2f}" if user_currency == "BDT" else f"+${total_payout_usd:.4f}"
...
def handle_pay_withdraw(message):
    ...
    try:
        ...
        if curr == "USDT":
            amt_str = f"${net:.4f}"
            pay_msg = f"✅ Withdrawal successful!\nAmount: {amt_str}"
            if txn_hash:
...
def handle_profile(message):
    ...
    cid  = message.chat.id
    user = get_user(cid)
    bal  = fmt_amount(user["balance"], get_currency(cid)) if user else "0.00 Tk"
    ref  = get_referral_count(cid)
    bot.send_message(cid,
        f"👤 {t(cid, 'আপনার প্রোফাইল', 'Your profile')}:\n"
        f"🆔 ID: `{cid}`\n"
        f"💰 {t(cid, 'ব্যালেন্স', 'Balance')}: {bal}\n"
        f"👥 {t(cid, 'রেফারেল', 'Referrals')}: {ref}"
    )
...
# FIX 3: Referral handler — referral_menu_inline এখন defined আছে তাই কাজ করবে
...
def handle_ref_leaderboard(call):
    ...
    for i, row in enumerate(rows):
        ...
        lines.append(f"{rank}. {uid_str} — {row['completed']} {'সাবমিশন' if is_bn(cid) else 'submissions'}")
...
def handle_top_leaderboard(message):
    ...
    lines = [
        ...
        f"ℹ️ {'ফলাফল প্রতিদিন ০১:০০ (Helsinki) তে ঘোষণা করা হয়।' if is_bn(cid) else 'Results announced daily at 01:00 Helsinki time.'}",
    ...
    if not rows:
        ...
        for i, row in enumerate(rows):
            ...
            lines.append(f"{rank}. {uid_str} — {row['completed']} {'সাবমিশন' if is_bn(cid) else 'submissions'} | {prize_str}")
...
def handle_reply_task_select(message):
    ...
    if is_bn(cid):
        info_text = (
            f"📋 *{meta['label']}*\n{divider()}\n"
            f"💰 পুরষ্কার: {price_display}/piece\n"
...
# FIX 2: Withdrawal callback — prefix "wmethod_" দিয়ে আলাদা করা হয়েছে
# এখন একটা button press করলে শুধু একটাই message আসবে
@bot.callback_query_handler(func=lambda c: c.data.startswith("wmethod_"))
def handle_withdraw_method(call):
    method_key = call.data[8:]  # "wmethod_" এর পরের অংশ
    bot.answer_callback_query(call.id)
    cid      = call.message.chat.id
    currency = get_currency(cid)
    ...
    # শুধু একবার info message দেখাবে
    bot.send_message(cid,
    ...
    if method_key == "usdt_bep":
        address_prompt = "📤 Enter your USDT (BEP-20) address:"
    elif method_key == "bkash":
        address_prompt = "📤 Enter your bKash number:"
    else:
        address_prompt = "📤 Enter your Nagad number:"
