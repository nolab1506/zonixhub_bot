            ...
            if ref_info and ref_info["referred_by"]:
                ...
                try:
                    ...
                    bot.send_message(referrer_id,
                        t(referrer_id,
                          f"🎁 *রেফারেল কমিশন!*\nআপনার রেফার করা ইউজার টাস্ক সম্পন্ন করায় ২০% কমিশন পেয়েছেন: *{comm_str}*",
                          f"🎁 *Referral Commission!*\nYour referred user completed a task. You earned: *{comm_str}*")
                    )
                except Exception:
                    pass
...
def handle_platform_instagram(message):
    ...
    currency = get_currency(cid)
    bot.send_message(cid,
        t(cid, "Instagram — টাস্ক সিলেক্ট করুন:", "Instagram — Select a task:"),
        reply_markup=instagram_type_inline(currency, cid)
    )
...
def handle_platform_facebook(message):
    ...
    currency = get_currency(cid)
    bot.send_message(cid,
        t(cid, "Facebook — টাস্ক সিলেক্ট করুন:", "Facebook — Select a task:"),
        reply_markup=facebook_type_inline(currency, cid)
    )
...
def handle_platform_gmail(message):
    ...
    currency = get_currency(cid)
    bot.send_message(cid,
        t(cid, "Gmail — টাস্ক সিলেক্ট করুন:", "Gmail — Select a task:"),
        reply_markup=gmail_type_inline(currency, cid)
    )
...
def handle_my_referrals(message):
    ...
    if is_bn(cid):
        bot.send_message(cid,
            ...
            f"ℹ️ আপনি প্রতিটি রেফারেলের আয়ের ২০% পাবেন।",
        ...
        bot.send_message(cid,
            ...
            f"ℹ️ You receive 20% of each referral's earnings.",
            reply_markup=referral_menu_inline(cid)
        )
